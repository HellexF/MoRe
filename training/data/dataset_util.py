...
def crop_image_depth_and_intrinsic_by_pp(
    image, depth_map, intrinsic, target_shape, track=None, motion=None, filepath=None, strict=False
):
    """
    ...
    if motion is not None:
        motion = motion[start_x:end_x, start_y:end_y]
    ...
    return image, depth_map, intrinsic, track, motion
...
def depth_to_world_coords_points(
    ...
    # Valid depth mask
    point_mask = (depth_map > eps) & (depth_map < 100.)
...
def depth_to_cam_coords_points(
    ...
    ), f"Intrinsic matrix must have zero skew {intrinsic}"
...
def rotate_90_degrees(
    image, depth_map, extri_opencv, intri_opencv, clockwise=True, track=None, motion=None
):
    """
    ...
    # Rotate moiton
    if motion is not None:
        rotated_motion = rotate_motion_rot90(motion)
    else:
        rotated_motion = None
...
def rotate_motion_rot90(motion, clockwise):
    """
    Rotates the given motion by 90 degrees (clockwise or counterclockwise),
    using a transpose+flip pattern.

    Args:
        motion (np.ndarray or None):
            Motion mask of shape (H, W), or None if not available.
        clockwise (bool):
            If True, rotate 90 degrees clockwise; else 90 degrees counterclockwise.

    Returns:
        rotated_motion (np.ndarray or None):
            Rotated motion mask
    """
    rotated_motion  = None
    if clockwise:
        if motion is not None:
            rotated_motion = np.transpose(motion, (1, 0))
            rotated_motion = np.flip(rotated_motion, axis=1)
    else:
        if motion is not None:
            rotated_motion = np.transpose(motion, (1, 0))
            rotated_motion = np.flip(rotated_motion, axis=0)
    return np.copy(rotated_motion)

...
class BaseDataset(Dataset):
    ...
    def process_one_image(
        ...
        image, depth_map, intri_opencv, track, motion = crop_image_depth_and_intrinsic_by_pp(
            image, depth_map, intri_opencv, aug_size, track=track, motion=motion, filepath=filepath,
        )
        ...
        if self.rescale:
            image, depth_map, intri_opencv, track, motion = resize_image_depth_and_intrinsic(
                image, depth_map, intri_opencv, target_shape, original_size, track=track, motion=motion,
        ...
        image, depth_map, intri_opencv, track, motion = crop_image_depth_and_intrinsic_by_pp(
            image, depth_map, intri_opencv, target_shape, track=track, motion=motion, filepath=filepath, strict=True,
        )
        ...
        if rotate_to_portrait:
            ...
            image, depth_map, extri_opencv, intri_opencv, track, motion = rotate_90_degrees(
                image,
                depth_map,
                extri_opencv,
                intri_opencv,
    ...
    def sample_fixed_interval_ids(self, num_images, img_per_seq, max_gap= 5):
        assert img_per_seq <= num_images

        while True:
            gap = np.random.randint(0, max_gap + 1)
            max_start_idx = num_images - (img_per_seq - 1) * (gap + 1) - 1
            if max_start_idx < 0:
                continue
            start_idx = np.random.randint(0, max_start_idx + 1)
            ids = np.arange(start_idx, start_idx + (gap + 1) * img_per_seq, gap + 1)
            if len(ids) >= img_per_seq:
                return ids[:img_per_seq]
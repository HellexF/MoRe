...
class ComposedDataset(Dataset, ABC):
    ...
    def __getitem__(self, idx_tuple):
        ...
        motions = None
        if batch["motions"]:
            motions = torch.from_numpy(np.stack(batch["motions"]).astype(np.int64))
        ...
        # logging.info(f"Processing sequence {seq_name} with {len(images)} images, ids {ids}, depth {np.unique(depths)} shaped {depths.shape}, invalid point mask sum {point_masks.numel() - point_masks.sum().item()}")

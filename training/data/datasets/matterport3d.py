...
class Matterport3DDataset(BaseDataset):
    def __init__(
        ...
        Args:
            ...
            MATTERPORT3D_DIR (str): Directory path to Matterport3D data.
            min_num_images (int): Minimum number of images per sequence.
            len_train (int): Length of the training dataset.
            len_test (int): Length of the test dataset.
        Raises:
            ValueError: If MATTERPORT3D_DIR is not specified.
        """
        ...
        if MATTERPORT3D_DIR is None:
            raise ValueError("MATTERPORT3D_DIR must be specified.")
        ...
        self.sequence_list = os.listdir(MATTERPORT3D_DIR)
        if split == "test":
        ...
        logging.info(f"MATTERPORT3D_DIR is {MATTERPORT3D_DIR}")
        self.MATTERPORT3D_DIR = MATTERPORT3D_DIR

        self.sequence_list_len = len(self.sequence_list)
        self.total_frame_num = 0
        for seq_name in self.sequence_list:
            self.total_frame_num += len(os.listdir(os.path.join(self.MATTERPORT3D_DIR, seq_name, "rgb")))
        ...
        logging.info(f"{status}: Matterport3D Data size: {self.sequence_list_len}")
    ...
    def get_data(
            ...
            base_frame_path = os.path.join(self.MATTERPORT3D_DIR, seq_name, "rgb")
            base_depth_path = os.path.join(self.MATTERPORT3D_DIR, seq_name, "depth")
            base_cam_path = os.path.join(self.MATTERPORT3D_DIR, seq_name, "cam")
            ...
            for global_idx in ids:
                image_path = osp.join(base_frame_path, f"{global_idx:06d}.png")
                image = read_image_cv2(image_path)

                depth_path = os.path.join(base_depth_path, f"{global_idx:06d}.npy")
                ...
                cam_path = osp.join(base_cam_path, f"{global_idx:06d}.npz")
                cam_file = np.load(cam_path)
                intri_opencv = cam_file["intrinsics"]
                extri_opencv = np.linalg.inv(cam_file["pose"])[:3, :]

...
class HOI4DDataset(BaseDataset):
    def __init__(
        ...
        Args:
            ...
            HOI4D_DIR (str): Directory path to Dynamic Replica data.
        ...
        if HOI4D_DIR is None:
            raise ValueError("HOI4D_DIR must be specified.")
        ...
        base = os.path.abspath(os.path.join(HOI4D_DIR, "HOI4D_release"))
        pattern = os.path.join(base, "ZY2021*", "H*", "C*", "N*", "S*", "s*", "T*")

        abs_list = glob.glob(pattern)
        self.sequence_list = [os.path.relpath(os.path.abspath(p), start=base) for p in abs_list]
        self.sequence_list = [seq for seq in self.sequence_list if os.path.exists(os.path.join(HOI4D_ANNO_DIR, seq.replace('/', '_'), "prior_depth")) and os.path.exists(os.path.join(HOI4D_ANNO_DIR, seq.replace('/', '_'), "camera")) and len(os.listdir(os.path.join(HOI4D_ANNO_DIR, seq.replace('/', '_'), "prior_depth"))) == 300]
        if split == "test":
            self.sequnce_list = [p for self.sequnce_list in "C20" in p]
        if self.debug:
            self.sequence_list = ['ZY20210800001/H1/C1/N44/S99/s02/T1']
        self.split = split   
        ...
        logging.info(f"HOI4D_DIR is {HOI4D_DIR}")
        logging.info(f"HOI4D_ANNO_DIR is {HOI4D_DIR}")
        self.HOI4D_DIR = HOI4D_DIR
        self.HOI4D_ANNO_DIR = HOI4D_ANNO_DIR

        self.sequence_list_len = len(self.sequence_list)
        self.total_frame_num = self.sequence_list_len * 300
        ...
        logging.info(f"{status}: HOI4D Data size: {self.sequence_list_len}")
        logging.info(f"{status}: HOI4D Data dataset length: {len(self)}")
    ...
    def get_data(
            ...
            base_frame_path = os.path.join(self.HOI4D_DIR, "HOI4D_release", seq_name, "align_rgb", "frames")
            base_depth_path = os.path.join(self.HOI4D_ANNO_DIR, seq_name.replace('/', '_'), "prior_depth")
            cam_path = os.path.join(self.HOI4D_ANNO_DIR, seq_name.replace('/', '_'), "camera", "recon", "split_0", "info.json")
            with open(cam_path, 'r') as f:
                info_data = json.load(f)

            # Extrinsics are provided as a list of 4x4 world-to-camera matrices (OpenCV convention)
            extri = torch.tensor(info_data['extrinsics'])  # Shape: (num_frames, 4, 4)
            
            num_frames = extri.shape[0]
            ...
            if ids is not None:
                ...
                assert num_frames > img_per_seq, f"seq {seq_name} has only {num_frames} images"
                if self.debug:
                    ids = np.arange(6)
                else:
            ...
            for global_idx in ids:
                image_path = osp.join(base_frame_path, f'{global_idx:06d}.jpg')
                image = read_image_cv2(image_path)

                depth_path = os.path.join(base_depth_path, f'{global_idx:05d}.png')
                ...
                motion_path = osp.join(self.HOI4D_DIR, "HOI4D_annotations", seq_name, '2Dseg', 'mask', f'{global_idx:05d}.png')
                if not os.path.exists(motion_path):
                    motion = np.zeros_like(depth_map)
                else:
                    motion = cv2.imread(motion_path)
                    motion = np.any(motion[..., :3] != 0, axis=2).astype(np.uint8) 

                original_size = np.array(image.shape[:2])
                extri_opencv = np.linalg.inv(extri[global_idx])[:3, :]
                extri_opencv[:, 3] /= 1000
                # extri_opencv = np.eye(4, 4)
                intri_opencv = intri

...
class BlendedMVSDataset(BaseDataset):
    def __init__(
        ...
        Raises:
            ValueError: If BLENDEDMVS_DIR is not specified.
        """
        ...
        self.sequence_list = [seq for seq in os.listdir(BLENDEDMVS_DIR) if not seq.startswith("000")]
        if split == "test":
        ...
        for seq_name in self.sequence_list:
            self.total_frame_num += len(os.listdir(os.path.join(self.BLENDEDMVS_DIR, seq_name, "rendered_depth_maps")))
        ...
        logging.info(f"{status}: BlendedMVS Data frame nums: {self.total_frame_num}")
    ...
    def get_data(
            ...
            base_frame_path = os.path.join(self.BLENDEDMVS_DIR, seq_name)

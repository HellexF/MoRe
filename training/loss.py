...
class MultitaskLoss(torch.nn.Module):
    ...
    """
    def __init__(self, camera=None, depth=None, motion=None, point=None, track=None, attn=None, dup_camera=None, **kwargs):
        super().__init__()
    ...
    def forward(self, predictions, batch) -> torch.Tensor:
        ...
        # Motion mask estimation loss
        if "motion" in predictions:
            motion_loss_dict = compute_motion_loss(predictions, batch, **self.motion)
            motion_loss = motion_loss_dict["loss_ce_motion"]
            motion_loss = motion_loss * self.motion["weight"]
            total_loss = total_loss + motion_loss
            loss_dict.update(motion_loss_dict)
        if self.attn:
            attn_loss_dict = compute_attn_loss(predictions, batch)
            attn_loss = attn_loss_dict["loss_kl_attn"]
            attn_loss = attn_loss * self.attn["weight"]
            total_loss = total_loss + attn_loss
            loss_dict.update(attn_loss_dict)
        # Camera pose loss - if pose encodings are predicted
        if "pose_enc_list" in predictions:
            camera_loss_dict = compute_camera_loss(predictions, batch, **self.camera, detach=True)  
            camera_loss = camera_loss_dict["loss_camera"] * self.camera["weight"]   
            total_loss = total_loss + camera_loss
            loss_dict.update(camera_loss_dict)
        
        if "dup_pose_enc_list" in predictions:
            dup_camera_loss_dict = compute_camera_loss(predictions, batch, **self.camera)  
            dup_camera_loss = dup_camera_loss_dict["loss_dup_camera"] * self.dup_camera["weight"]   
            total_loss = total_loss + dup_camera_loss
            loss_dict.update(dup_camera_loss_dict)
...
def compute_attn_loss(predictions, batch):
    motions = batch["motions"]
    B, N, H_mask, W_mask = motions.shape
    patch_size = 14 

    motion_imgs = motions.to(torch.float32).view(B * N, H_mask, W_mask)
    gt_scores = F.avg_pool2d(motion_imgs, kernel_size=patch_size, stride=patch_size)
    gt_scores = gt_scores.flatten(start_dim=1)
    # gt_scores = gt_scores / (gt_scores.sum(dim=-1, keepdim=True))
    gt_scores = gt_scores.unsqueeze(1).expand(-1, 16, -1)

    attn_weight = predictions["attn_weight"]
    if attn_weight.shape[2] == 2:
        gt_scores = gt_scores.unsqueeze(2).expand(-1, -1, 2, -1)
    else:
        gt_scores = gt_scores.unsqueeze(2)

    # loss_kl = kl_loss(attn_weight, gt_scores)
    loss_kl = (gt_scores * attn_weight).mean()


    loss_dict = {
        f"loss_kl_attn": loss_kl,    
    }
...
def compute_camera_loss(
    ...
    relative_loss,          # whether to compuet relative camera loss
    detach = False,
    ...
    weight_rot=2.0,         # weight for rotation loss
    weight_focal=0.5,       # weight for focal length loss
    **kwargs
):
    if not detach:
        pred_pose_encodings = pred_dict['dup_pose_enc_list']
    else:
    ...
    depths = batch_data['depths'][:, 0]
    valid_frame_mask = point_masks[:, 0].sum(dim=[-1, -2]) > 100
    max_hw = depths.amax(dim=(-1, -2))
    min_hw = depths.amin(dim=(-1, -2))
    eps = 1e-2
    mask = (max_hw < (-1 + eps)) & (min_hw > (-1 - eps))
    valid_frame_mask = valid_frame_mask | mask
    n_stages = len(pred_pose_encodings)

    gt_extrinsics = batch_data['extrinsics']  # (B, 4, 4)
    ...
    def pose_encoding_to_T(pose_enc: torch.Tensor) -> torch.Tensor:
        """
        pose_enc: [B, N, 9]  ->  [B, N, 4, 4]
                [qw, qx, qy, qz, tx, ty, tz, scale]
        """
        B, N, _ = pose_enc.shape

        # 1. normalize quaternion
        q = pose_enc[..., 3:7] # [B, N, 4]

        # 2. quaternion -> rotation matrix [B, N, 3, 3]
        R = quat_to_mat(q)
        # 3. translation [B, N, 3]
        t = pose_enc[..., 0:3]

        # 5. build homogeneous matrix [B, N, 4, 4]
        T = torch.zeros(B, N, 4, 4, device=pose_enc.device, dtype=pose_enc.dtype)
        T[..., :3, :3] = R
        T[..., :3, 3] = t
        T[..., 3, 3] = 1.0
    ...
    total_loss_R = total_loss_T = total_loss_FL = 0
    for stage_idx in range(n_stages):
        ...
        if relative_loss:
            if valid_frame_mask.sum() == 0:
                ...
                pred_valid = pred_pose_stage[valid_frame_mask].clone()   # [Bv, N, 9]
                gt_valid   = gt_pose_encoding[valid_frame_mask].clone()  # [Bv, N, 9]
                Bv, N, _ = pred_valid.shape

                pred_T = pose_encoding_to_T(pred_valid)   # [Bv, N, 4, 4]
                gt_T   = pose_encoding_to_T(gt_valid)     # [Bv, N, 4, 4]

                pred_Ti = pred_T.unsqueeze(2).expand(Bv, N, N, 4, 4)
                pred_Tj = pred_T.unsqueeze(1).expand(Bv, N, N, 4, 4)
                gt_Ti   = gt_T.unsqueeze(2).expand(Bv, N, N, 4, 4)
                gt_Tj   = gt_T.unsqueeze(1).expand(Bv, N, N, 4, 4)

                if detach:
                    idx_i = torch.arange(N, device=pred_T.device).view(1, N, 1, 1, 1)
                    idx_j = torch.arange(N, device=pred_T.device).view(1, 1, N, 1, 1)
                    idx_i = idx_i.expand(Bv, N, N, 1, 1)
                    idx_j = idx_j.expand(Bv, N, N, 1, 1)

                    pred_Ti_mod = torch.where(idx_i < idx_j, pred_Ti.detach(), pred_Ti)
                    pred_Tj_mod = torch.where(idx_i > idx_j, pred_Tj.detach(), pred_Tj)

                    T_hat_ij = torch.inverse(pred_Ti_mod) @ pred_Tj_mod
                else:
                    T_hat_ij = torch.inverse(pred_Ti) @ pred_Tj   # [Bv, N, N, 4, 4]
                T_gt_ij  = torch.inverse(gt_Ti)   @ gt_Tj     # [Bv, N, N, 4, 4]

                R_hat = T_hat_ij[..., :3, :3]  # [Bv, N, N, 3, 3]
                t_hat = T_hat_ij[..., :3, 3]   # [Bv, N, N, 3]
                R_gt  = T_gt_ij[..., :3, :3]
                t_gt  = T_gt_ij[..., :3, 3]

                loss_R_all = F.l1_loss(R_hat, R_gt, reduction="none").mean(dim=(-1, -2))    

                # ---- Translation loss (Huber) ----
                loss_T_all = F.l1_loss(t_hat, t_gt, reduction="none").sum(-1)  # [Bv, N, N]

                mask = ~torch.eye(N, dtype=torch.bool, device=pred_T.device).unsqueeze(0).expand(Bv, -1, -1)
                loss_R_stage = loss_R_all[mask]
                loss_T_stage = loss_T_all[mask]

                if loss_type == "l1":
                    loss_FL_stage = (pred_valid[..., 7:] - gt_valid[..., 7:]).abs()
                elif loss_type == "l2":
                    loss_FL_stage = (pred_valid[..., 7:] - gt_valid[..., 7:]).norm(dim=-1)

                loss_T_stage = check_and_fix_inf_nan(loss_T_stage, "loss_T").mean()
                loss_R_stage = check_and_fix_inf_nan(loss_R_stage, "loss_R").mean()
                loss_FL_stage = check_and_fix_inf_nan(loss_FL_stage, "loss_FL").mean()
    ...
    total_camera_loss = weight_rot * avg_loss_R + weight_trans * avg_loss_T # + weight_focal * avg_loss_FL

    if not detach:
        return {
            "loss_dup_camera": total_camera_loss,
            "loss_dup_R": avg_loss_R,
            "loss_dup_T": avg_loss_T,
            "loss_dup_FL": avg_loss_FL
        }
    return {
...
def compute_motion_loss(predictions, batch, gamma=1.0, alpha=0.2, gradient_loss_fn=None, valid_range=-1, **kwargs):
    """
    Compute motion loss.
    
    Args:
        predictions: Dict containing 'motion' and 'motion_conf'
        batch: Dict containing ground truth 'motion'
        gamma: Weight for confidence loss
        alpha: Weight for confidence regularization
        gradient_loss_fn: Type of gradient loss to apply
        valid_range: Quantile range for outlier filtering
    ...
    gt_motion = batch['motions']
    gt_motion = check_and_fix_inf_nan(gt_motion, "gt_motion")
    gt_motion = gt_motion[..., None]              # (B, H, W, 1)

    loss_ce = cross_entropy_loss(pred_motion, gt_motion,
                                            gamma=gamma, alpha=alpha, valid_range=valid_range)

    loss_dict = {
        f"loss_ce_motion": loss_ce,    
    }
...
def regression_loss(pred, gt, mask, conf=None, gradient_loss_fn=None, gamma=1.0, alpha=0.2, valid_range=-1):
    ...
    #loss_conf_punish = alpha * torch.log(conf[~mask])
    #loss_conf_punish = check_and_fix_inf_nan(loss_conf_punish, "loss_conf_punish")
    ...
    if loss_conf.numel() > 0:
        ...
        if valid_range>0:
            ...
            #loss_conf_punish = filter_by_quantile(loss_conf_punish, valid_range)

        loss_conf = check_and_fix_inf_nan(loss_conf, f"loss_conf_depth")
        loss_conf = loss_conf.mean()
        #loss_conf = loss_conf + 0.1 * loss_conf_punish.mean()
    else:
...
def cross_entropy_loss(
    pred,          # (B, S, H, W, 1) logits for binary classification
    gt,            # (B, S, H, W) ground truth labels, 0 or 1
    gamma=1.0,     # confidence weighting
    alpha=0.2,     # confidence regularization
    valid_range=-1 # quantile filtering
):
    """
    Binary classification loss with confidence weighting and optional outlier filtering.
 
    Args:
        pred: (B, S, H, W, 1) logits
        gt: (B, S, H, W) ground truth labels, 0 or 1
        conf: (B, S, H, W) confidence weights (optional)
        gamma: weight for confidence term
        alpha: weight for confidence regularization
        valid_range: if >0, apply quantile filtering

    Returns:
        loss_bce: standard BCE loss
    """
    B, S, H, W, C = pred.shape
    assert C == 1, f"Expected C==1 for binary classification, got {C}"

    pred = pred.reshape(-1)             # (N,)
    gt = gt.reshape(-1).float()         # (N,), must be float for BCE

    # Binary cross entropy with logits
    loss_bce = F.binary_cross_entropy_with_logits(pred, gt, reduction='none')  # (N,)
    loss_bce = check_and_fix_inf_nan(loss_bce, "loss_ce_motion")

    # Quantile filtering
    if valid_range > 0:
        loss_bce = filter_by_quantile(loss_bce, valid_range)

    # Mean or zero fallback
    if loss_bce.numel() > 0:
        loss_bce = loss_bce.mean()
    else:
        loss_bce = (0.0 * pred).mean()

    return loss_bce

def kl_loss(attn, scores, sign=+1, eps=1e-6, var_thr=1e-8):
    if sign > 0:
        kl = (scores * (scores.log() - attn.log())).sum(dim=-1)
    else:
        kl = (attn * (attn.log() - scores.log())).sum(dim=-1)

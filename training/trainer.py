...
class Trainer:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.checkpoint_conf.get('resume_checkpoint_path', None) is not None:
        ...
        elif self.checkpoint_conf.get('load_checkpoint_path', None) is not None:
            self.load_weights_only(self.checkpoint_conf.load_checkpoint_path)
    ...
    def _load_resuming_checkpoint(self, ckpt_path: str):
        ...
        if 'additional_camera_token' not in model_state_dict:
            model_state_dict['aggregator.additional_camera_token'] = model_state_dict['aggregator.camera_token'].clone()

        # filtered_dict = {k: v for k, v in model_state_dict.items() if k.startswith("aggregator.")}
        # model_state_dict.update(filtered_dict)
        ignore_prefixes = getattr(self.checkpoint_conf, "ignore", None)
        if ignore_prefixes:
        ...
        if "optimizer" in checkpoint and self.mode != 'val':
            ...
            if isinstance(self.optims, list):
                if isinstance(optim_state, list):
                    assert len(optim_state) == len(self.optims), \
                        f"Mismatch in number of optimizers: {len(optim_state)} vs {len(self.optims)}"
                    for opt_obj, opt_state in zip(self.optims, optim_state):
                        opt_obj.optimizer.load_state_dict(opt_state)
                else:
                    # Single optimizer case, apply to the first
                    assert len(self.optims) == 1, \
                        f"Expected one optimizer, but got {len(self.optims)}"
                    self.optims[0].optimizer.load_state_dict(optim_state)
            else:
                self.optims.optimizer.load_state_dict(optim_state)
    ...
    def load_weights_only(self, ckpt_path: str):
        """
        Load only the model weights from a checkpoint.
        - Does NOT load optimizer, scaler, steps/epoch, or time info.
        - Supports prefix-based key ignoring via self.checkpoint_conf.ignore.
        """
        logging.info(f"Loading weights from {ckpt_path} (rank {getattr(self, 'rank', 'NA')})")
        ...
        try:
            ...
            logging.error(f"Failed to read checkpoint {ckpt_path}: {e}")
            raise

        # pick model state dict
        model_state = (
            ckpt.get("model", None)
        ...
        if not isinstance(model_state, dict):
            raise RuntimeError(f"Checkpoint does not contain a valid state dict: keys={list(ckpt.keys())}")

        # optional: ignore keys by prefix
        ignore_prefixes = getattr(self.checkpoint_conf, "ignore", None)
        if ignore_prefixes:
        ...
        # load into model
        strict = getattr(self.checkpoint_conf, "strict", True)
        missing, unexpected = self.model.load_state_dict(model_state, strict=strict)

        if getattr(self, "rank", 0) == 0:
            logging.info(
                f"Model weights loaded. Missing: {missing or 'None'}; Unexpected: {unexpected or 'None'}."
            )

        # DO NOT touch optimizer/scaler/epoch/steps/time
        return True
    ...
    def run_train(self):
        ...
        while self.epoch < self.max_epochs:
            ...
            # val_loader = self.val_dataset.get_loader(epoch=int(self.epoch))
            ...
            #     self.run_val(val_loader)

            self.epoch += 1

        self.epoch -= 1

    def run_val(self, dataloader=None):
    ...
    #     phase = 'val'
    #     loss_keys = self._get_scalar_log_keys(phase)
    #     loss_names = [f"Loss/{phase}_{name}" for name in loss_keys]
    #     loss_meters = {
    #         name: AverageMeter(name, self.device, ":.4f") for name in loss_names
    #     }

    #     total_loss_sums = {name: 0.0 for name in loss_names}
    #     total_samples = 0
    ...
    #         with torch.cuda.amp.autocast(enabled=self.optim_conf.amp.enabled, dtype=amp_type):
    ...
    #         for k in loss_keys:
    #             name = f"Loss/{phase}_{k}"
    #             total_loss_sums[name] += val_loss_dict[k].item() * batch_size
    ...
    #     avg_losses = {name: total_loss_sums[name] / total_samples for name in loss_names}

    #     log_str = [f"{name}: {avg_losses[name]:.4f}" for name in loss_names]
        
    #     for name, meter in loss_meters.items():
    #         logging.info(f"[Val] {name}: {meter.avg:.4f}")
    #     print(f"==== Validation Summary Epoch {self.epoch} ====")
    #     for name, meter in loss_meters.items():
    #         print(f"{name}: {meter.avg:.4f}")
    #         self.tb_writer.log(name, meter.avg, self.steps["val"])
    ...
    def train_epoch(self, train_loader):        
        ...
        for data_iter, batch in enumerate(train_loader):
            ...
            if total_t > 100.0: 
                logging.warning(f"[SlowBatch] iter={data_iter} seq name={batch["seq_name"]} total={total_t:.2f}s "
                f"(process={t_process:.2f}s, copy={t_copy:.2f}s, forward={t_forward:.2f}s)")
    ...
    def _apply_batch_repetition(self, batch: Mapping) -> Mapping:
        ...
        """
        tensor_keys = [
            "images", "depths", "motions", "extrinsics", "intrinsics", 

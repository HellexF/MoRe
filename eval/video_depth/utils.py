...
def save_depth_maps(depth_maps, path, conf_self=None):
    ...
    if conf_self is not None:
        ...
        min_conf = torch.log(conf_self.min())  # float(torch.quantile(out, 0.01))
        max_conf = torch.log(conf_self.max())  # float(torch.quantile(out, 0.99))

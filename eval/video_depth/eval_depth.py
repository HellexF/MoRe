...
def get_args_parser():
    ...
    parser.add_argument("--max_depth", type=int, default=70, help="pytorch device")
    return parser
...
def main(args):
    if args.eval_dataset == "sintel":
        ...
        if full:
            depth_pathes = glob.glob(f"/lpai/volumes/base-3da-ali-sh-mix/zhangwq/dataset/vggt/MPI-Sintel-depth-training-20150305/training/depth/*/*.dpt")
            ...
            depth_pathes_folder = [
                f"/lpai/volumes/base-3da-ali-sh-mix/zhangwq/dataset/vggt/MPI-Sintel-depth-training-20150305/training/depth/{seq}" for seq in seq_list
            ]
            depth_pathes = []
        ...
        def get_video_results():
            ...
            depth_log_txt = f"{args.output_dir}/result_{args.align}.txt"
            average_metrics = {
                key: np.average(
            ...
            with open(depth_log_txt, "w") as f:
                f.write(
                    "AbsRel, LogRMSE, δ<1.25\n"
                    f"{average_metrics['Abs Rel']:.5f},{average_metrics['Log RMSE']:.5f},{average_metrics['δ < 1.25']:.5f}"
                )
        ...
        img_pathes_folder = [
            f"/lpai/volumes/base-3da-ali-sh-mix/zhangwq/dataset/vggt/rgbd_bonn_dataset/rgbd_bonn_{seq}/rgb_110/*.png"
            for seq in seq_list
        ]
        img_pathes = []
        ...
        depth_pathes_folder = [
            f"/lpai/volumes/base-3da-ali-sh-mix/zhangwq/dataset/vggt/rgbd_bonn_dataset/rgbd_bonn_{seq}/depth_110/*.png"
            for seq in seq_list
        ]
        depth_pathes = []
        for depth_pathes_folder_i in depth_pathes_folder:
        ...
        img_pathes_folder = [
            f"/lpai/volumes/base-3da-ali-sh-mix/zhangwq/dataset/vggt/TUM/{seq}/rgb_90/*.png"
            for seq in seq_list
        ]
        img_pathes = []
        for img_pathes_folder_i in img_pathes_folder:
        ...
        img_pathes = sorted(img_pathes)
        depth_pathes_folder = [
            f"/lpai/volumes/base-3da-ali-sh-mix/zhangwq/dataset/vggt/TUM/{seq}/depth_90/*.png"
            for seq in seq_list
        ]
        depth_pathes = []
        for depth_pathes_folder_i in depth_pathes_folder:
        ...
        depth_pathes = glob.glob(
            "/lpai/dataset/action-model-group-data-weight/0-1-0/datasets/cut3r_data/eval_data/kitti/depth_selection/val_selection_cropped/groundtruth_depth_gathered/*/*.png"
        )
        depth_pathes = sorted(depth_pathes)
        pred_pathes = glob.glob(
            f"{args.output_dir}/*/frame_*.npy"
